...
void relxill_convolution_multizone(const XspecSpectrum &spectrum,
                                   ...
                                   int *status);
void relxill_convolution_multizone_emis(const XspecSpectrum &spectrum,
                                   const relline_spec_multizone_emis *rel_profile,
                                   const SpectrumZones_emis &xill_spec_zones,
                                   specCache *spec_cache,
                                   const relParam *rel_param,
                                   const CachingStatus &caching_status,
                                   int *status);                              
...
static void check_caching_energy_grid(CachingStatus &caching_status, specCache *cache, const XspecSpectrum &spectrum) {
  ...
  if (cache->out_spec == nullptr) {
    ...
    if (cache->out_spec->nbins != spectrum.num_flux_bins()) {
      return;
    }
...
void copy_spectrum_to_cache(const XspecSpectrum &spectrum,
  ...
  if (spec_cache->out_spec != nullptr) {
    if (spec_cache->out_spec->nbins != spectrum.num_flux_bins()) {
      free_spectrum(spec_cache->out_spec);
      spec_cache->out_spec = new_spectrum(spectrum.num_flux_bins(), spectrum.energy);
    }
  } else {
    spec_cache->out_spec = new_spectrum(spectrum.num_flux_bins(), spectrum.energy);
  }
...
void relxill_kernel(const XspecSpectrum &spectrum,
  ...
  if (caching_status.is_all_cached()) { // already cached, simply use the cached output flux value
    ...
    relline_spec_multizone_emis *rel_profile =
        relbase_profile_emis(ener_conv, n_ener_conv, rel_param, sys_par, xill_tab,
                        ion_gradient.radial_grid.radius, ion_gradient.nzones(), status);

    // --- 5c --- calculate the xillver spectra depending on the angular distribution (stored in the rel_profile)

    auto xillver_spectra_zones =  SpectrumZones_emis(spec_cache->xill_spec[0]->ener, spec_cache->xill_spec[0]->n_ener, rel_param->num_zones, spec_cache->xill_spec[0]->n_incl);

    for (int ii=0; ii <  rel_param->num_zones; ii++){
      calc_xillver_angdep_emis(xillver_spectra_zones.flux[ii],
                          spec_cache->xill_spec[ii],
                          rel_profile->rel_cosne->dist[ii],
                          spec_cache->xill_spec[0]->n_incl,
                          status);
      ...
      for (int kk =0 ; kk<xillver_spectra_zones.num_emis; kk++){
        for (int jj = 0; jj < xillver_spectra_zones.num_flux_bins; jj++) {
          xillver_spectra_zones.flux[ii][kk][jj] *= norm_change;
        }
      }
    }
    ...
    relxill_convolution_multizone_emis(spectrum,
                                  rel_profile,
  ...
  if (is_debug_run()){
    char filename[256];
    sprintf(filename, "/Users/ym.huang/Downloads/relyimin/relxill_yimin%.0f.txt", rel_param->incl*180/M_PI);
    save_xillver_spectrum(spectrum.energy, spectrum.flux, spectrum.num_flux_bins(), filename);
  }
  delete rel_param;
  delete xill_param;
}
...
void relxill_convolution_multizone_emis(const XspecSpectrum &spectrum,
  ...
  for (int ii = 0; ii < rel_profile->n_zones; ii++) { /***** loop over ionization zones   ******/
    ...
    for(int jj=0; jj<rel_profile->rel_cosne->n_cosne; jj++){

      /** avoid problems where no relxill bin falls into an ionization bin **/
      if (calcSum(rel_profile->flux[ii][jj], rel_profile->n_ener) < 1e-12) {
        continue;
      }

      for (int kk =0; kk<n_ener_conv;kk++){
        xill_rebinned_spec[kk]=0.0;
      }

      rebin_spectrum(ener_conv, xill_rebinned_spec, n_ener_conv,
                    xill_spec_zones.energy() , xill_spec_zones.flux[ii][jj], xill_spec_zones.num_flux_bins);

      // --2-- convolve the spectrum on the energy grid "ener_conv" **
      int recompute_xill = 1; // always recompute fft for xillver, as relat changes the angular distribution
      convolveSpectrumFFTNormalized(ener_conv, xill_rebinned_spec, rel_profile->flux[ii][jj], conv_out, n_ener_conv,
                                    caching_status.recomput_relat(), recompute_xill, ii, spec_cache, status);
      CHECK_STATUS_VOID(*status);
      rebin_spectrum(spectrum.energy, single_spec_inp, spectrum.num_flux_bins(), ener_conv, conv_out, n_ener_conv);
