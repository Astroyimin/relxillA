...
void renorm_relline_profile_emis(relline_spec_multizone_emis *spec, relParam *rel_param, const int *status) {
  ...
  for (ii = 0; ii < spec->n_zones; ii++) {
    for (kk=0; kk < spec->rel_cosne->n_cosne; kk++){
      for (jj = 0; jj < spec->n_ener; jj++) {
        spec->flux[ii][kk][jj] /= 0.5 * (spec->ener[jj] + spec->ener[jj + 1]);
        sum += spec->flux[ii][kk][jj];
      }
    }
  }
    ...
    for (ii = 0; ii < spec->n_zones; ii++) {
      for (kk=0; kk < spec->rel_cosne->n_cosne; kk++){
        for (jj = 0; jj < spec->n_ener; jj++) {
          if (sum<1e-12){
            if (is_debug_run()){
              std::cout<<"Warning**: Devide zero error in renorm"<<std::endl;
            }
          }
          spec->flux[ii][kk][jj] *= relline_norm / sum;
        }
      }
    }
  }
...
void calc_relline_profile_emis(relline_spec_multizone_emis *spec, RelSysPar *sysPar, int *status) {
  ...
  for (ii = 0; ii < sysPar->nr; ii++) {
    ...
    if ((egmax > spec->ener[0]) && (egmin < spec->ener[spec->n_ener])) {
      ...
      for (jj = ielo; jj <= iehi; jj++) {
        for (kk=0; kk<2;kk++){
          double eg = (spec->ener[jj] + spec->ener[jj + 1]) / 2.0;
          double egstar = ( eg- da->gmin) * da->del_g;
          int ind;
          if (!((egstar >= da->gstar[da->save_g_ind]) && (egstar < da->gstar[da->save_g_ind + 1]))) {
            da->save_g_ind = binary_search(da->gstar, da->ng, egstar);
          }
          ind = da->save_g_ind;
          int imu = get_cosne_bin(da->cosne[ind][kk], spec->rel_cosne);
          double tmp_var = integ_relline_bin_emis(cached_str_relb_func, spec->ener[jj], spec->ener[jj + 1],kk);
          spec->flux[izone][imu][jj] += tmp_var * weight;
        }
      }
