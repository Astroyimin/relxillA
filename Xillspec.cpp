...
void calc_xillver_angdep_emis(double **xill_flux, xillSpec *xill_spec, const double *dist,int n_emis,const int *status) {

  CHECK_STATUS_VOID(*status);

  int ii;
  int jj;
  for (int jj; jj<n_emis; jj++){
    for (ii = 0; ii < xill_spec->n_ener; ii++) {
      xill_flux[jj][ii] = 0.0;
    }
  }
  ...
  for (ii = 0; ii < xill_spec->n_incl; ii++) {
    for (jj = 0; jj < xill_spec->n_ener; jj++) {
      xill_flux[ii][jj] += xill_spec->flu[ii][jj];
    }
  }
