...
class SpectrumZones_emis{

 public:
  SpectrumZones_emis(const double* _energy, int _num_flux, int _num_zones, int _num_emis):
      num_flux_bins(_num_flux), num_zones(_num_zones), num_ener_bins(_num_flux+1),num_emis(_num_emis) {

    flux = new double**[num_zones];
    for (int ii=0; ii<num_zones; ii++){
      flux[ii] = new double*[num_emis];
      for (int kk=0; kk<num_emis; kk++){
        flux[ii][kk] = new double[num_flux_bins];
        for (int jj=0; jj<num_flux_bins; jj++) {
          flux[ii][kk][jj] = 0.0;
        }
      }
