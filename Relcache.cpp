...
cnode *add_node_to_cache_emis(cnode *head, relParam *relpar, xillParam *xillpar, int *status) {
...
void add_relspec_to_cache_emis(cnode **node, relParam *param, relline_spec_multizone_emis *spec, int *status) {
  ...
  // prepend new node and set parameters
  cnode *new_head = add_node_to_cache_emis(old_head, param, nullptr, status);

  // set the data
  new_head->data->relbase_spec_emis = spec;
...
cdata *init_cdata(int *status) {
  ...
  data->relbase_spec_emis = nullptr;
...
static void free_cdata(cdata **pt_data) {
  ...
  if (*pt_data != nullptr) {
    ...
    free_rel_spec_emis(data->relbase_spec_emis);
