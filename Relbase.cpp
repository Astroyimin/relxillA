...
relline_spec_multizone_emis* relbase_profile_emis(double *ener, int n_ener, relParam *param,
                                       RelSysPar *sysPar,
                                       xillTable *xill_tab,
                                       const double *radialZones,
                                       int nzones,
  ...
  inpar* inp = get_inputvals_struct(ener, n_ener, param, status);
  cache_info *ca_info = cli_check_cache(cache_relbase, inp, check_cache_relpar, status);
  relline_spec_multizone_emis *spec = nullptr;
  ...
  if (is_relbase_cached((ca_info)) == 0) {
    ...
    // init the spectra where we store the flux
    param->num_zones = nzones;
    init_relline_spec_multizone_emis(&spec, param, xill_tab, radialZones, &ener, n_ener, status);

    calc_relline_profile_emis(spec, sysPar, status); // returned units are 'photons/bin'
    ...
    // normalize it and calculate the angular distribution (if necessary)
    renorm_relline_profile_emis(spec, param, status);

    // last step: store parameters and cached relline_spec_multizone (this prepends a new node to the cache)
    add_relspec_to_cache_emis(&cache_relbase, param, spec, status);
    if (is_debug_run() && *status == EXIT_SUCCESS) {
      printf(" DEBUG:  Adding new RELBASE eval to cache; the count is %i \n", cli_count_elements(cache_relbase));
    }
  } else {
    ...
    spec = ca_info->store->data->relbase_spec_emis;
  }
  free(inp);
  free(ca_info);
...
void free_rel_spec_emis(relline_spec_multizone_emis *spec) {
  if (spec != nullptr) {
    free(spec->ener);
    free(spec->rgrid);
    if (spec->flux != nullptr) {
      ...
      for (ii = 0; ii < spec->n_zones; ii++) {
        if (spec->flux[ii] != nullptr) {
          ...
          for (jj=0; jj < spec->rel_cosne->n_cosne; jj++){
            if (spec->flux[ii][jj] != nullptr) {
              free(spec->flux[ii][jj]);
            }
          }
          free(spec->flux[ii]);
        }
...
Spectrum *new_spectrum(int num_flux_bins, const double *ener) {

  auto *spec = new Spectrum;
  spec->nbins = num_flux_bins;
  spec->ener = new double[num_flux_bins + 1];
  spec->flux = new double[num_flux_bins];

  for (int ii = 0; ii < num_flux_bins; ii++) {
    spec->ener[ii] = ener[ii];
    spec->flux[ii] = 0.0;
  }
  spec->ener[num_flux_bins] = ener[num_flux_bins];
